...
SHT3xHandler::SHT3xHandler(uint8_t address)
{
  ...
  avgCounter = 0;
  tmpAvgHumidity = avgHumidity = humidity = 0.0f;
  tmpAvgCTemp = avgCTemp = cTemp = 0.0f;
  avgValuesAvailable = false;
  state = STATE_SEND_COMMAND;
  brzo_i2c_setup( SDA, SCL, 0 );
}

void SHT3xHandler::handle( time_t timestamp )
{
  if ( state == STATE_SEND_COMMAND )
  {
    if (( timestamp - intervalTimestamp ) >= UPDATE_INTERVAL )
    {
  ...
  if( state == STATE_READ_DATA )
  {
    if (( millis() - intervalTimestamp ) >= 15l )
    {
      ...
      if ( error == 0 )
      {
        ...
        tmpAvgCTemp += cTemp;
        tmpAvgHumidity += humidity;
        avgCounter++;

        // LOG1( "SHT3x avg counter=%d\n", avgCounter );

        if ( avgCounter == AVG_NUM_INTERVALS )
        {
          avgCounter = 0;
          avgValuesAvailable = true;
          avgCTemp = tmpAvgCTemp / AVG_NUM_INTERVALS;
          avgHumidity = tmpAvgHumidity / AVG_NUM_INTERVALS;
          tmpAvgCTemp = tmpAvgHumidity = 0.0f;
        }
